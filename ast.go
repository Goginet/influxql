...
func CloneExpr(expr Expr) Expr {
	...
	return func() Expr { e, _ := CloneExprWithConstraints(context.Background(), 0, expr); return e }()
}

func CloneExprWithConstraints(ctx context.Context, maxDepth int, expr Expr) (Expr, error) {
	return cloneExprWithConstraints(ctx, 0, maxDepth, expr)
}

func cloneExprWithConstraints(ctx context.Context, depth int, maxDepth int, expr Expr) (Expr, error) {
	if expr == nil {
		return nil, nil
	}
	...
	case *BinaryExpr:
		lhs, err := cloneExprWithConstraints(ctx, depth+1, maxDepth, expr.LHS)
		if err != nil {
			return nil, err
		}
		rhs, err := cloneExprWithConstraints(ctx, depth+1, maxDepth, expr.RHS)
		if err != nil {
			return nil, err
		}
	...
	case *Call:
		...
		for i, arg := range expr.Args {
			e, err := cloneExprWithConstraints(ctx, depth+1, maxDepth, arg)
			if err != nil {
				return nil, err
			}
			args[i] = e
	...
	case *ParenExpr:
		e, err := cloneExprWithConstraints(ctx, depth+1, maxDepth, expr.Expr)
		if err != nil {
			return nil, err
		}
